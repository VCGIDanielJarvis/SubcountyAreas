...
`AGGREGATION_TARGET_COUNTS = [60, 89, 100, 107, 150]` (or the closest count it actually
reaches, §3.5).
...
| 3. Merge loop | Each round: find every touching pair, rank by combined population + `MaxDiffK` similarity, merge the current lowest-population area with its best-ranked own neighbor. | Guarantees the population floor is addressed every round; ranking (not raw differences) keeps one extreme pair elsewhere on the map from distorting every other comparison. |
